...
SELECT CONCAT_WS(" ", surname, firstname, CASE WHEN facid = 0 THEN 'Tennis Court 1'
            WHEN facid = 1 THEN 'Tennis Court 2'
            ELSE 'FAILURE' END) AS test
FROM Bookings
LEFT JOIN Members
...
WHERE EXTRACT(YEAR FROM starttime) = 2012
      ...
      AND CASE WHEN m.memid != 0 THEN slots*membercost 
            ELSE slots*guestcost END > 30
ORDER BY cost DESC
...
FROM (SELECT name AS facility_name,
      CASE WHEN m.memid != 0 THEN CONCAT_WS(" ", surname, firstname) 
...
SELECT COUNT(CASE WHEN b.memid != 0 THEN 1
                      ELSE 0 END),
           f.name
    FROM BOOKINGS AS b
    LEFT JOIN Facilities AS f
